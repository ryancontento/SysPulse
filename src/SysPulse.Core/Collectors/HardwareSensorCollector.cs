...
internal sealed record SensorReading(string? Name, double? LoadPercent, double? TemperatureC, double? ClockMhz, double? FanRpm)
{
    public static SensorReading None { get; } = new(null, null, null, null, null);
}

/// <summary>
/// Temperatures, clocks, and fans via LibreHardwareMonitor. CPU and motherboard sensors need
/// elevation; without it those readings come back null.
/// </summary>
internal sealed class HardwareSensorCollector : IDisposable
{
    ...
    private static SensorReading ReadCpu(IHardware? cpu, IHardware? motherboard)
    {
        ...
        var temperature = FindByName(sensors, SensorType.Temperature, CpuTemperatureSensors)
            ?? (double?)sensors.FirstOrDefault(s => s.SensorType == SensorType.Temperature && s.Value.HasValue)?.Value;

        var clock = sensors
            .Where(s => s.SensorType == SensorType.Clock && s.Value.HasValue && s.Name.StartsWith("Core #", StringComparison.Ordinal))
        ...
        return new SensorReading(cpu.Name, null, temperature, clock, fan);
    }
